...
eurostat = eurostat[eurostat["geo"].isin(["BG", "PL", "CZ", "HU", "SK","RO","SI","HR"])]
...
orbis_HR = pd.read_excel("/Users/eugenmojsovic/Desktop/Teza/CapitalIQ/HR.xlsx")
orbis_HR = orbis_HR.replace(["N.A.", "n.a.", "NA", "na", ""], np.nan)
orbis_HR_clean = orbis_HR.dropna()
orbis_HR_filtered = orbis_HR_clean[orbis_HR_clean["Last avail. year"] == 2024]
orbis_HR_filtered = orbis_HR_filtered.apply(lambda col: pd.to_numeric(col.str.replace('.', '', regex=False), errors='coerce') if col.dtype == 'object' and col.name != 'Name' else col)
orbis_HR_filtered["ind"] = orbis_HR_filtered["NACE Rev. 2, core code (4 digits)"].astype(str).str[:2]
orbis_HR_filtered["ind"] = orbis_HR_filtered["ind"].apply(map_nace_2digit)
orbis_HR_filtered = orbis_HR_filtered[orbis_HR_filtered["ind"] != "Finance & insurance"]
orbis_HR_filtered = orbis_HR_filtered.drop(columns=["Last avail. year","NACE Rev. 2, core code (4 digits)"])
orbis_HR_filtered.columns = [convert_year(c) for c in orbis_HR_filtered.columns]
orbis_HR_filtered["country"] = "HR"
orbis_HR_filtered["gas"]=0
for year in range(2024, 2018, -1):
    fa_y   = f"Tangible fixed assets EUR {year}"
    fa_y1  = f"Tangible fixed assets EUR {year - 1}"
    ta_y   = f"Total assets EUR {year}"
    col_name = f"investment_proxy_{year}"
    orbis_HR_filtered[col_name] = (
        (orbis_HR_filtered[fa_y] - orbis_HR_filtered[fa_y1]) /
        orbis_HR_filtered[ta_y])
    sg_col = f"sales_g {year}"
    rev_t = f"Operating revenue (Turnover) EUR {year}"
    rev_t1 = f"Operating revenue (Turnover) EUR {year - 1}"
    orbis_HR_filtered[sg_col] = (
        (orbis_HR_filtered[rev_t] - orbis_HR_filtered[rev_t1]) /
        orbis_HR_filtered[rev_t1])
    er_col = f"er {year}"
    sf_t = f"Shareholders funds EUR {year}"
    orbis_HR_filtered[er_col] = (
        orbis_HR_filtered[sf_t] /
        orbis_HR_filtered[ta_y])
orbis_HR_filtered["sales_g 2018"] = 0  
orbis_HR_filtered["investment_proxy_2018"] = 0
for year in range(2018, 2025):
    ebit_col = f"Operating profit (loss) [EBIT] EUR {year}"
    ta_col   = f"Total assets EUR {year}"
    prof_col = f"profitability {year}"
    orbis_HR_filtered[prof_col] = (
    orbis_HR_filtered[ebit_col] /
    orbis_HR_filtered[ta_col])
    size_col = f"Size {year}"
    orbis_HR_filtered[size_col] = np.log(orbis_HR_filtered[ta_col])   
orbis_HR_filtered = orbis_HR_filtered.replace([np.inf, -np.inf], np.nan)
orbis_HR_filtered = orbis_HR_filtered.dropna(subset=[size_col])
orbis_HR_filtered = (
    orbis_HR_filtered
    .nlargest(1000, size_col)
    .copy())
...
for year in range(2019, 2025):
    ...
    cols_to_keep = base_cols + year_cols
    df_year = orbis_HR_filtered[cols_to_keep].copy()
    df_year["post"] = 1 if year >= 2022 else 0
    df_year["year"] = year
    df_year["post_gas"] = df_year["post"] * df_year["gas"]
    df_year = df_year.rename(columns=lambda c: (
    ...
    datasets_by_yearHR[year] = df_year
HR = pd.concat(datasets_by_yearHR.values(), ignore_index=True)
HR["Alternative"] = 1
HR['alt_post_gas'] = HR['post_gas'] * HR['Alternative']
HR["post_alt"] = HR["post"] * HR["Alternative"]
    
all_datasets = [BG_total, PL_total, CZ_total, RO_total, SK_total, SI_total, HU_total,HR]
wcro = pd.concat(all_datasets, ignore_index=True)
wcro["Alternative"] = wcro["country"].isin(["PL", "BG", "RO"]).astype(int)
wcro = wcro.fillna(0)
wcro['investment_proxy'] = winsorize(wcro['investment_proxy'], limits=[0.001, 0.001])
wcro['sales_g'] = winsorize(wcro['sales_g'], limits=[0.001, 0.001])
...
file_path = os.path.join("/Users/eugenmojsovic/Desktop/Teza/", "wcro.csv")
wcro.to_csv(file_path, index=False)
